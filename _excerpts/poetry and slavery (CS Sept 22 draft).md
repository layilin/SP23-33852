...
What we are calling the "full text" version contains the entire poem on a single page. In other words, readers can enjoy a continuous reading experience by scrolling through the edition without ever having to click backward or forward on their browser. This version of the poem also makes it easier to perform global keyword searches. To see all the uses of a particular word in the poem, readers can either use their browser's "Find" command, or use the "Search" function included on our site's sidebar. This version of the poem also makes it easier for readers to print or save a copy of the poem for consultation offline. We recommend first using the browser's "Print" command to create a PDF of the full text, and then either saving or printing the file.
